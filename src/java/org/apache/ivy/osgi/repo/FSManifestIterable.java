...
public class FSManifestIterable extends AbstractFSManifestIterable<File> {
     ...
     */
    public static final FilenameFilter DEFAULT_DIR_FILTER = (File dir, String name) -> !name.equals(".svn") && !NON_BUNDLE_DIRS.contains(name);

    /**
     * Default bundle filter that select only .jar files
     */
    public static final FilenameFilter DEFAULT_BUNDLE_FILTER = (File dir, String name) -> name.endsWith(".jar");
    ...
    protected List<File> listBundleFiles(File dir) throws IOException {
        return Arrays.asList(dir.listFiles((File f) -> f.isFile() && bundleFilter.accept(f.getParentFile(), f.getName())));
    }

    protected List<File> listDirs(File dir) throws IOException {
        return Arrays.asList(dir.listFiles((File f) -> f.isDirectory() && (dirFilter == null || dirFilter.accept(f.getParentFile(), f.getName()))));
    }
}
